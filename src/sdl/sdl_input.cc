...
static void IN_DeactivateMouse( void )
{
	...
	if( mouseActive )
	{
		...
		if( SDL_GetWindowFlags( SDL_window ) & SDL_WINDOW_MOUSE_FOCUS )
			SDL_WarpMouseInWindow( SDL_window, cls.vidconfig.vidWidth / 2, cls.vidconfig.vidHeight / 2 );
...
void IN_Frame (void) {
	...
	if( !cls.vidconfig.isFullscreen && ( Key_GetCatcher( ) & KEYCATCH_CONSOLE ) )
	{
		// Console is down in windowed mode
		IN_DeactivateMouse( );
	}
	else if( !cls.vidconfig.isFullscreen && loading )
	{
