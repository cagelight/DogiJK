...
q3world::q3worldmesh_maplit::q3worldmesh_maplit(vertex_t const * data, size_t num, mode m) : q3mesh_basic(m) {
	...
	static constexpr uint_fast8_t offsetof_lm_idx = offsetof_color + sizeof_color;
	static constexpr uint_fast8_t sizeof_lm_idx = sizeof(vertex_t::lm_idx);
	static constexpr uint_fast8_t offsetof_lm_uv = offsetof_lm_idx + sizeof_lm_idx;
	...
	glEnableVertexArrayAttrib(m_handle, LAYOUT_LMIDX);
	...
	glVertexArrayAttribBinding(m_handle, LAYOUT_LMIDX, 0);
	...
	glVertexArrayAttribIFormat(m_handle, LAYOUT_LMIDX, 4, GL_UNSIGNED_SHORT, offsetof_lm_idx);
