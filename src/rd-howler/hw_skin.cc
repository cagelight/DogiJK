...
q3skin_ptr instance::skin_registry::reg(char const * name, bool server) {
	if (!name || !name[0]) return 0;
	
	auto const & find = lookup.find(name);
...
q3skin_ptr instance::skin_registry::get(qhandle_t h) {
	if (h < 0 || h >= static_cast<int32_t>(skins.size())) return nullptr;
	return skins[h];
}
