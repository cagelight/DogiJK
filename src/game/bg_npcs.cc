...
extern int NPC_WeaponsForTeam( team_t team, int spawnflags, istring const & NPC_type );
void NPC_PrecacheWeapons( team_t playerTeam, int spawnflags, istring const & NPCtype )
{
...
void NPC_Precache ( gentity_t *spawner )
{
	...
	if ( !Q_stricmp( "random", spawner->NPC_type.c_str() ) )
	{//sorry, can't precache a random just yet
		return;
	}
	...
	Com_sprintf( sessionName, sizeof(sessionName), "NPC_Precache(%s)", spawner->NPC_type.c_str() );
...
qboolean NPC_ParseParmsActual( istring const & NPCNameIn, gentity_t *NPC, istring & playerModel, istring & customSkin, gNPCstats_t * stats, qboolean & parsingPlayer, qboolean & md3Model )
{
	const char	*token;
	const char	*value;
	const char	*p;
	...
	istring NPCName = NPCNameIn;

	int fp;
	
	while (true) {
		auto npcm = NPCMetaParms.find(NPCName);
		if (npcm == NPCMetaParms.end()) break;
		...
		std::vector<std::pair<istring, float>> NPCOptions;
		...
		while ( 1 )
		{
			...
			istring name = token;
			COM_ParseFloat(&p, &weight);
			
			NPCOptions.emplace_back(std::move(name), weight);
		}
		
		if (NPCOptions.size()) {
			float sum = 0;
			for (auto const & [str, w] : NPCOptions) sum += w;
			float rnd = Q_flrand(0, sum);
			for (auto const & [str, w] : NPCOptions) {
				NPCName = str;
				rnd -= w;
				if (rnd <= 0) break;
			}
...
//===MODEL PROPERTIES===========================================================
		if ( !Q_stricmp( token, "base" ) )
		{
			...
			qboolean status = NPC_ParseParmsActual(value, NPC, playerModel, customSkin, stats, parsingPlayer, md3Model);
			if (!status) return qfalse;
			continue;
		}
...
//===MISC===============================================================================
		...
		if ( !Q_stricmp( token, "snd" ) )
		{
			...
			if ( !(NPC->r.svFlags&SVF_NO_BASIC_SOUNDS) )
			{
				...
				NPC->s.csSounds_Std = G_SoundIndex(va("*$%s", sound));
			}
			continue;
		}
		...
		if ( !Q_stricmp( token, "sndcombat" ) )
		{
			...
			if ( !(NPC->r.svFlags&SVF_NO_COMBAT_SOUNDS) )
			{
				...
				NPC->s.csSounds_Combat = G_SoundIndex(va("*$%s", sound));
			}
			continue;
		}
		...
		if ( !Q_stricmp( token, "sndextra" ) )
		{
			...
			if ( !(NPC->r.svFlags&SVF_NO_EXTRA_SOUNDS) )
			{
				...
				NPC->s.csSounds_Extra = G_SoundIndex(va("*$%s", sound));
			}
			continue;
		}
		...
		if ( !Q_stricmp( token, "sndjedi" ) )
		{
			...
			if ( !(NPC->r.svFlags&SVF_NO_EXTRA_SOUNDS) )
			{
				...
				NPC->s.csSounds_Jedi = G_SoundIndex(va("*$%s", sound));
			}
			continue;
		}
		...
		if ( !Q_stricmp( token, "saber" ) )
		{
			...
			NPC->s.npcSaber1 = G_ModelIndex(va("@%s", saberName));
		...
		if ( !Q_stricmp( token, "saber2" ) )
		{
			...
			{//can't use a second saber if first one is a two-handed saber...?
				...
				else
				{
					...
					NPC->s.npcSaber2 = G_ModelIndex(va("@%s", saberName));
				}
				BG_TempFree(4096);
			}
			continue;
...
extern void SetupGameGhoul2Model(gentity_t *ent, char const *modelname, char const *skinName);
qboolean NPC_ParseParms( const char *NPCNameIn, gentity_t *NPC )
{
	istring playerModel, customSkin = "default";
	renderInfo_t	*ri = &NPC->client->renderInfo;
...
*/
	...
	*/
	...
	Com_sprintf( sessionName, sizeof(sessionName), "NPC_ParseParms(%s)", NPCName.c_str() );
	COM_BeginParseSession(sessionName);
	if (!NPC_ParseParmsActual(NPCName, NPC, playerModel, customSkin, stats, parsingPlayer, md3Model)) return qfalse;
...
*/
	if ( !md3Model )
	{
		...
		if (NPC->s.npcSaber1 == 0)
		{ //use "kyle" for a default then
			NPC->s.npcSaber1 = G_ModelIndex("@Kyle");
			WP_SaberParseParms( DEFAULT_SABER, &NPC->client->saber[0] );
		}

		if (!customSkin.size() || !customSkin[0])
		{
			customSkin = "default";
		}
		...
		{ //vehicles want their names fed in as models
			...
			playerModel = va("$%s", NPCName.c_str());
		}
		SetupGameGhoul2Model(NPC, playerModel.c_str(), customSkin.c_str());

		if (!NPC->NPC_type.size())
...
void BG_NPCLoadParms( void )
{
	...
	npcExtFNLen = 0;
	memset(npcExtensionListBuf, 0, sizeof(npcExtensionListBuf));
	fileCnt = trap->FS_GetFileList("ext_data/NPCs", ".npcmeta", npcExtensionListBuf, sizeof(npcExtensionListBuf) );
